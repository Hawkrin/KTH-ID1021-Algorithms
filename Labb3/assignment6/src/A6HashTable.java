 ...
 * The Hashtable is based upon princetons HashTable.java, but modified.
 * 
 */
public class A6HashTable <Key, Value> {
     ...
     */
    public A6HashTable() { this(INIT_CAPACITY); } 
    ...
    public A6HashTable(int tableSize) {
    ...
    private void resize(int chains) {
        A6HashTable<Key, Value> temp = new A6HashTable<Key, Value>(chains);
        for (int i = 0; i < tableSize; i++) {
    ...
    public void put(Key key, Value value) {
        if (key == null) { throw new IllegalArgumentException("fthe key is null"); }
        if (value == null) {
            delete(key);
            return;
        }
